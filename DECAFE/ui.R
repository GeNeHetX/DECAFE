    ...
    tabPanel("Boxplot",
      fluidRow(column(width=3,uiOutput('geneTarget')),
        ...
        fluidRow(
              column(
                width = 4,
                # SelectInput pour l'ordre des conditions
                selectInput(
                  inputId = "order_boxplot",
                  label = "Choose order of conditions in boxplot:",
                  choices = NULL,   # sera rempli dynamiquement côté server
                  multiple = TRUE
                )
              ),
              column(
